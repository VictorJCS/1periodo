...
void printMatrix(int matrix[ROWS][COLS], int itemCount) {
    ...
    printf("Número de itens coletados: %d\n", itemCount);
}

void placeItems(int matrix[ROWS][COLS], int *itemCount, int numItems) {
    int posX, posY;
    for (int i = 0; i < numItems; i++) {
        printf("Digite a posição do item %d (linha coluna): ", i + 1);
        scanf("%d %d", &posX, &posY);

        while(posX < 0 || posX >= ROWS || posY < 0 || posY >= COLS || matrix[posX][posY] != 0) {
            printf("Posição inválida para o item %d! Digite novamente: ", i + 1);
            scanf("%d %d", &posX, &posY);
        }
...
int main() {
    ...
    do {
        ...
        int posX, posY;
        printf("Escolha a posição inicial do robô (linha coluna): ");
        scanf("%d %d", &posX, &posY);

        while(posX < 0 || posX >= ROWS || posY < 0 || posY >= COLS) {
            printf("Posição inicial inválida! Digite novamente: ");
            scanf("%d %d", &posX, &posY);
        }
        ...
        printf("Quantos itens você deseja colocar? ");
        scanf("%d", &numItems);
        ...
        while (itemCount < numItems) {
            ...
            if (itemCount == numItems) {
                printf("\nTodos os itens foram coletados! Parabéns!\n");
                break;
            }
        }
