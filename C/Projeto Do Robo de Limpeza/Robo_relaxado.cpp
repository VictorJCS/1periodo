...
void moveRobot(int *posX, int *posY, int targetX, int targetY, int matrix[ROWS][COLS], Position *path, int *pathLength) {
    while (*posX != targetX || *posY != targetY) {
        ...
        matrix[*posX][*posY] = 3; // Atualiza a posição para vazio
    ...
    for (int i = 0; i < ROWS; i++) {
        ...
        for (int j = 0; j < COLS; j++) {
            if (i == initialPosX && j == initialPosY)
                ...
                printf("|%c|", matrix[i][j] == 0 ? '-' : ' '); // Imprime '-' apenas para células vazias
        }
        printf("\n");
    }
}
...
int main() {
    ...
    do {
        ...
        if (itemCount == numItems) {
            ...
            clearPath(matrix, path, pathLength); // Limpa o caminho percorrido pelo robô
            matrix[initialPosX][initialPosY] = 0; // Marca a posição inicial como vazia
            matrix[posX][posY] = 1; // Marca a posição final como a estação
            system("cls");
            printMatrixWithStation(matrix, initialPosX, initialPosY); // Imprime a matriz com a estação na posição final
        } else {
